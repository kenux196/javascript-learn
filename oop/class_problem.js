...
const cIveMembers = iveMembers.map(
  (x) => new FemaleIdolModel(x['name'], x['year']),
);
console.log(cIveMembers);

const cBtsMembers = btsMembers.map(
  (x) => new MaleIdolModel(x['name'], x['year']),
);
console.log(cBtsMembers);

const ive = new IdolGroup('IVE', cIveMembers);
console.log(ive);

const bts = new IdolGroup('BTS', cBtsMembers);
console.log(bts);
...
const allTogether = new Country('대한민국', [
  new IdolGroup(
    'IVE',
    iveMembers.map((x) => new FemaleIdolModel(x['name'], x['year'])),
  ),
  new IdolGroup(
    'BTS',
    btsMembers.map((x) => new FemaleIdolModel(x['name'], x['year'])),
  ),
]);
